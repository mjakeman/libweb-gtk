...
public:
    explicit ContentViewImpl(WebContentView *widget, StringView webdriver_content_ipc_path, WebView::EnableCallgrindProfiling, WebView::UseJavaScriptBytecode);
    virtual ~ContentViewImpl() override;

    Function<String(const AK::URL&, Web::HTML::ActivateTab)> on_tab_open_request;

    void snapshot_vfunc(GtkSnapshot *snapshot);
