...
static void
signal_show_event(ContentViewImpl *impl)
{
    impl->show_event();
}

static void
signal_hide_event(ContentViewImpl *impl)
{
    impl->hide_event();
}

ContentViewImpl::ContentViewImpl(WebContentView *widget, StringView webdriver_content_ipc_path, WebView::EnableCallgrindProfiling enable_callgrind_profiling, WebView::UseJavaScriptBytecode use_javascript_bytecode)
        : m_webdriver_content_ipc_path(webdriver_content_ipc_path)
        , m_widget(widget)
{
    gtk_widget_set_focusable(GTK_WIDGET(m_widget), true);
    gtk_widget_set_can_focus(GTK_WIDGET(m_widget), true);

    m_device_pixel_ratio = (float) gtk_widget_get_scale_factor(GTK_WIDGET(m_widget));
    m_inverse_pixel_scaling_ratio = 1.0f / m_device_pixel_ratio;

    // TODO: Adjustments !!
//    property_vadjustment().signal_changed().connect([&]() {
//        get_vertical_adj() = property_vadjustment().get_value();
//        get_vertical_adj()->property_value().signal_changed().connect([&]() {
//            update_viewport_rect();
//        });
//    });
//
//    property_hadjustment().signal_changed().connect([&]() {
//        get_horizontal_adj() = property_hadjustment().get_value();
//        get_horizontal_adj()->property_value().signal_changed().connect([&]() {
//            update_viewport_rect();
//        });
//    });

    g_signal_connect_swapped (m_widget, "map", G_CALLBACK (signal_show_event), this);
    g_signal_connect_swapped (m_widget, "unmap", G_CALLBACK (signal_hide_event), this);

    // Event Controllers
    m_motion_controller = Gtk::EventControllerMotion::create();
    m_motion_controller->signal_motion().connect(sigc::mem_fun(*this, &ContentViewImpl::on_motion));
    gtk_widget_add_controller(GTK_WIDGET(m_widget), GTK_EVENT_CONTROLLER (m_motion_controller->gobj()));

    m_key_controller = Gtk::EventControllerKey::create();
    m_key_controller->set_propagation_phase(Gtk::PropagationPhase::BUBBLE);
    m_key_controller->signal_key_pressed().connect(sigc::mem_fun(*this, &ContentViewImpl::on_key_pressed), false);
    m_key_controller->signal_key_released().connect(sigc::mem_fun(*this, &ContentViewImpl::on_key_released), false);
    gtk_widget_add_controller(GTK_WIDGET(m_widget), GTK_EVENT_CONTROLLER (m_key_controller->gobj()));

    m_click_gesture = Gtk::GestureClick::create();
    m_click_gesture->signal_pressed().connect(sigc::mem_fun(*this, &ContentViewImpl::on_pressed), false);
    m_click_gesture->signal_released().connect(sigc::mem_fun(*this, &ContentViewImpl::on_release), false);
    gtk_widget_add_controller(GTK_WIDGET(m_widget), GTK_EVENT_CONTROLLER (m_click_gesture->gobj()));
...
bool ContentViewImpl::on_key_pressed(guint keyval, guint, Gdk::ModifierType state)
{
//    switch (keyval) {
//        case GDK_KEY_Left:
//        case GDK_KEY_Right:
...
void ContentViewImpl::on_key_released(guint keyval, guint, Gdk::ModifierType state)
{
    gunichar point = gdk_keyval_to_unicode(keyval);
    auto key = translate_keyval(keyval);
    auto modifiers = translate_modifiers(state);
...
void ContentViewImpl::on_pressed(int n_press, double x, double y)
{
...
void ContentViewImpl::on_release(int n_press, double x, double y)
{
    if (n_press > 1)
        return;
...
void ContentViewImpl::on_motion(double x, double y)
{
    Gfx::IntPoint position(x / m_inverse_pixel_scaling_ratio, y / m_inverse_pixel_scaling_ratio);
...
/*void ContentViewImpl::dragEnterEvent(QDragEnterEvent* event)
{
    if (event->mimeData()->hasUrls())
        event->acceptProposedAction();
}

void ContentViewImpl::dropEvent(QDropEvent* event)
{
...
void ContentViewImpl::focusInEvent(QFocusEvent*)
{
    client().async_set_has_focus(true);
}

void ContentViewImpl::focusOutEvent(QFocusEvent*)
{
    client().async_set_has_focus(false);
}*/

void ContentViewImpl::resize_event(int, int)
{
    update_viewport_rect();
    handle_resize();
}
...
void ContentViewImpl::size_allocate_vfunc(int width, int height, int)
{
    resize_event(width, height);
}

void ContentViewImpl::snapshot_vfunc(GtkSnapshot* snapshot)
{
    int width = gtk_widget_get_allocated_width(GTK_WIDGET (m_widget));
    int height = gtk_widget_get_allocated_width(GTK_WIDGET (m_widget));
    const Gdk::Rectangle rect(0, 0, width, height);

    gtk_snapshot_scale(snapshot, m_inverse_pixel_scaling_ratio, m_inverse_pixel_scaling_ratio);
    ...
    if (bitmap) {
        ...
        graphene_rect_init(&local_rect, (float)0, (float)0, (float)bitmap_size.width(), (float)bitmap->height());
        auto cr = gtk_snapshot_append_cairo(snapshot, &local_rect);
        ...
        auto cairo_bitmap = cairo_image_surface_create_for_data(dest, CAIRO_FORMAT_RGB24, bitmap->width(), bitmap->height(), 4 * bitmap->width());
        cairo_set_source_surface(cr, cairo_bitmap, 0, 0);
        cairo_paint(cr);

        if (bitmap_size.width() < rect.get_width()) {
            graphene_rect_init(&local_rect, (float)bitmap_size.width(), (float)0, (float)(width - bitmap_size.width()), (float)(bitmap->height()));
            gtk_snapshot_append_color(snapshot, &white, &local_rect);
        }
        if (bitmap_size.height() < rect.get_height()) {
            graphene_rect_init(&local_rect, (float)0, (float)bitmap_size.height(), (float)width, (float)(height - bitmap_size.height()));
            gtk_snapshot_append_color(snapshot, &white, &local_rect);
        }
...
void ContentViewImpl::set_viewport_rect(Gfx::IntRect rect)
{
    m_viewport_rect = rect;
    client().async_set_viewport_rect(rect);
}

void ContentViewImpl::set_window_size(Gfx::IntSize size)
{
    client().async_set_window_size(size);
}

void ContentViewImpl::set_window_position(Gfx::IntPoint position)
{
    client().async_set_window_position(position);
}

GtkAdjustment* ContentViewImpl::get_horizontal_adj() const
{
    return gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(m_widget));
}

GtkAdjustment* ContentViewImpl::get_vertical_adj() const
{
    return gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(m_widget));
}

void ContentViewImpl::update_viewport_rect()
{
    auto scaled_width = int((float)gtk_widget_get_width(GTK_WIDGET (m_widget)) / m_inverse_pixel_scaling_ratio);
    auto scaled_height = int((float)gtk_widget_get_height(GTK_WIDGET (m_widget)) / m_inverse_pixel_scaling_ratio);
...
void ContentViewImpl::update_zoom()
{
...
void ContentViewImpl::show_event()
{
    client().async_set_system_visibility_state(true);
}

void ContentViewImpl::hide_event()
{
    client().async_set_system_visibility_state(false);
}

static Core::AnonymousBuffer make_system_theme_from_gtk_palette(Gtk::Widget& widget, ContentViewImpl::PaletteMode mode)
{
    auto style_context = widget.get_style_context();

    auto theme_file = mode == ContentViewImpl::PaletteMode::Default ? "Default"sv : "Dark"sv;
...
void ContentViewImpl::update_palette(PaletteMode mode)
{
    auto widget = Glib::wrap(GTK_WIDGET (m_widget));
    client().async_update_system_theme(make_system_theme_from_gtk_palette(*widget, mode));
}

void ContentViewImpl::create_client(WebView::EnableCallgrindProfiling enable_callgrind_profiling, WebView::UseJavaScriptBytecode use_javascript_bytecode)
{
    m_client_state = {};
...
void ContentViewImpl::notify_server_did_paint(Badge<WebContentClient>, i32 bitmap_id, Gfx::IntSize size)
{
...
void ContentViewImpl::notify_server_did_invalidate_content_rect(Badge<WebContentClient>, [[maybe_unused]] Gfx::IntRect const& content_rect)
{
    request_repaint();
}

void ContentViewImpl::notify_server_did_change_selection(Badge<WebContentClient>)
{
    request_repaint();
}

void ContentViewImpl::notify_server_did_request_cursor_change(Badge<WebContentClient>, Gfx::StandardCursor cursor)
{
    switch (cursor) {
        case Gfx::StandardCursor::Hidden:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "none");
            break;
        case Gfx::StandardCursor::Arrow:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "default");
            break;
        case Gfx::StandardCursor::Crosshair:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "crosshair");
            break;
        case Gfx::StandardCursor::IBeam:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "text");
            break;
        case Gfx::StandardCursor::ResizeHorizontal:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "col-resize");
            break;
        case Gfx::StandardCursor::ResizeVertical:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "row-resize");
            break;
        case Gfx::StandardCursor::ResizeDiagonalTLBR:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "nwse-resize");
            break;
        case Gfx::StandardCursor::ResizeDiagonalBLTR:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "nesw-resize");
            break;
        ...
        case Gfx::StandardCursor::Hand:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "pointer");
            break;
        case Gfx::StandardCursor::Help:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "help");
            break;
        case Gfx::StandardCursor::Drag:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "grabbing");
            break;
        case Gfx::StandardCursor::DragCopy:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "copy");
            break;
        case Gfx::StandardCursor::Move:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "grabbing");
            break;
        case Gfx::StandardCursor::Wait:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "wait");
            break;
        case Gfx::StandardCursor::Disallowed:
            gtk_widget_set_cursor_from_name(GTK_WIDGET (m_widget), "not-allowed");
            break;
...
void ContentViewImpl::notify_server_did_request_scroll(Badge<WebContentClient>, i32 x_delta, i32 y_delta)
{
    auto h_adj = get_horizontal_adj();
    auto v_adj = get_horizontal_adj();

    if (h_adj) {
        int h_adj_value = (int) gtk_adjustment_get_value(h_adj);
        gtk_adjustment_set_value(h_adj, max(0, h_adj_value + x_delta));
    }

    if (v_adj) {
        int v_adj_value = (int) gtk_adjustment_get_value(v_adj);
        gtk_adjustment_set_value(v_adj, max(0, v_adj_value + y_delta));
    }
}

void ContentViewImpl::notify_server_did_request_scroll_to(Badge<WebContentClient>, Gfx::IntPoint scroll_position)
{
    auto h_adj = get_horizontal_adj();
    auto v_adj = get_horizontal_adj();

    if (h_adj) {
        gtk_adjustment_set_value(h_adj, scroll_position.x());
    }

    if (v_adj) {
        gtk_adjustment_set_value(v_adj, scroll_position.y());
    }
}

void ContentViewImpl::notify_server_did_request_scroll_into_view(Badge<WebContentClient>, Gfx::IntRect const& rect)
{
    ...
    if (v_adj && rect.top() < m_viewport_rect.top())
        gtk_adjustment_set_value(v_adj, rect.top());
    else if (v_adj && rect.top() > m_viewport_rect.top() && rect.bottom() > m_viewport_rect.bottom())
        gtk_adjustment_set_value(v_adj, rect.bottom() - m_viewport_rect.height());
}

void ContentViewImpl::notify_server_did_enter_tooltip_area(Badge<WebContentClient>, Gfx::IntPoint content_position, DeprecatedString const& tooltip)
{
    (void) content_position;
    gtk_widget_set_tooltip_text(GTK_WIDGET (m_widget), ustring_from_ak_deprecated_string(tooltip).c_str());
}

void ContentViewImpl::notify_server_did_leave_tooltip_area(Badge<WebContentClient>)
{
    gtk_widget_set_tooltip_text(GTK_WIDGET (m_widget), "");
}

void ContentViewImpl::notify_server_did_request_alert(Badge<WebContentClient>, String const& message)
{
...
void ContentViewImpl::notify_server_did_request_confirm(Badge<WebContentClient>, String const& message)
{
    m_dialog = Gtk::AlertDialog::create();
...
void ContentViewImpl::notify_server_did_request_prompt(Badge<WebContentClient>, String const& message, String const& default_)
{
    (void) message;
    (void) default_;
...
void ContentViewImpl::notify_server_did_request_set_prompt_text(Badge<WebContentClient>, String const& message)
{
    (void) message;
...
void ContentViewImpl::notify_server_did_request_accept_dialog(Badge<WebContentClient>)
{
//    if (m_dialog)
//        m_dialog->accept();
}

void ContentViewImpl::notify_server_did_request_dismiss_dialog(Badge<WebContentClient>)
{
//    if (m_dialog)
//        m_dialog->reject();
}

void ContentViewImpl::notify_server_did_request_file(Badge<WebContentClient>, DeprecatedString const& path, i32 request_id)
{
...
Gfx::IntRect ContentViewImpl::viewport_rect() const
{
    return m_viewport_rect;
}

Gfx::IntPoint ContentViewImpl::to_content_position(Gfx::IntPoint widget_position) const
{
    auto h_adj = get_horizontal_adj();
    int h_adj_value = h_adj != nullptr ? (int) gtk_adjustment_get_value(h_adj) : 0;
    auto v_adj = get_horizontal_adj();
    ...
    return widget_position.translated(max(0, h_adj_value), max(0, v_adj_value));
}

Gfx::IntPoint ContentViewImpl::to_widget_position(Gfx::IntPoint content_position) const
{
    auto h_adj = get_horizontal_adj();
    int h_adj_value = h_adj != nullptr ? (int) gtk_adjustment_get_value(h_adj) : 0;
    auto v_adj = get_horizontal_adj();
    ...
    return content_position.translated(-(max(0, h_adj_value)), -(max(0, v_adj_value)));
}

/*bool ContentViewImpl::event(QEvent* event)
{
...
void ContentViewImpl::notify_server_did_finish_handling_input_event(bool event_was_accepted)
{
...
ErrorOr<String> ContentViewImpl::dump_layout_tree()
{
